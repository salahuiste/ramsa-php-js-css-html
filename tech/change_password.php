<?php
	...
	if($_SERVER["REQUEST_METHOD"] == "POST") {
 	...
 	$donnes=$bdd->prepare("SELECT Pwd FROM agent WHERE Matricule=:mat");
 	...
 	if(strlen($_POST['newpassword1'])<5) {
 		...
 		if($thepass['Pwd']!=$thepass_entred OR strlen($thepass_entred)<5) {
 		...
 		if($_POST['newpassword1']!=$_POST['newpassword2']) {
 			...
 			$req = $bdd->prepare('UPDATE agent set Pwd=:pass WHERE Matricule=:mat');
